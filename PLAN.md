...
6. **Deploy** — done. Live at **https://personal-portfolio-omega-six-41.vercel.app** (Vercel project `mitrofan12a-2162/personal-portfolio`, no custom domain per section 5). Deployed with `vercel deploy --prod` from the CLI; `vercel.json` pins it as a pure static site (`framework: null`, `npm run generate` → `.output/public`). Lighthouse on the live URL, `/` and `/architecture`: Performance/Accessibility/Best Practices/SEO all 100. Two deploy-only bugs found and fixed: (a) Vercel makes Nitro auto-pick the `vercel-static` preset, which prerendered only 7 of 24 routes (no `/ru/*`, no i18n message files) — preset now pinned to `static` and all routes listed explicitly in `nuxt.config.ts`; (b) the static preset's `200.html` SPA fallback made unknown URLs return HTTP 200 (soft-404) — a `postgenerate` hook now deletes it so Vercel serves the real `404.html`. **Not connected:** Vercel's GitHub integration (auto-deploy on push) — Vercel refused because the GitHub account has no Login Connection; deploys are manual (`vercel deploy --prod`) until that's set up from the Vercel dashboard.
